...
namespace ETicaretAPI.Persistance.Services
{
    public class AuthorizationEndpointService : IAuthorizationEndpointService
    {
        ...
        public async Task AssignRoleEndpointAsync(string[] roles, string menu, string code, Type type)
        {
            ...
            if (endpoint == null)
            {
                var action = _applicationService.GetAuthorizeDefinitionEndpoints(type).FirstOrDefault(m => m.MenuName == menu)?.Actions.FirstOrDefault(e => e.Code == code);
        ...
        public async Task<List<string>> GetRolesToEndpointsAsync(string code,string menu)
        {
